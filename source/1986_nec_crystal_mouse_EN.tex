...
Like all early optical mice, this manipulator requires a reflective mat with a grid pattern. Unlike traditional Mouse Systems “mirror” mats with a grid of vertical and horizontal lines corresponding to two different wavelengths, the Nec Crystal Mouse's metal mat has a dark, glossy surface. (figure \ref{fig:NecCrystalPad}).
...
The manipulator's internal structure is shown on figure \ref{fig:NecCrystalInside}, where you can see the original backlighting of the optical mouse, which differs from most optical mice of the 1980s, which copied Mouse Systems designs. Light emitted by LEDs is reflected off the mouse pad, then re-reflected by mirrors inside the mouse (a pair of rectangular blocks with a blue outer surface in the center of the structure), and finally emitted to a series of photodetectors, one located along the longitudinal axis and the other along the transverse axis. In the Nec Crystal Mouse, the LEDs (and, consequently, the mirrors) are located at the outer corners of the mouse pad's surface.
...
Also, the figure \ref{fig:NecCrystalInside} allows us to conclude that the mouse is connected to the computer via a type of serial interface (this is evidenced by its cable, which consists of four wires, of which, according to the markings on the printed circuit board, two are responsible for receiving and transmitting data bits, and two provide power).
